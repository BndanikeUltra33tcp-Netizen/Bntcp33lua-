...
-- BOTÕES
-------------------------------------------------
CreateButton("otm mt boa", 0, function(state)
...
levelBtn.Text = "NÍVEL OTM: "..Settings.NoCollideNivel
...
levelBtn.MouseButton1Click:Connect(function()
	...
	levelBtn.Text = "Nível da otm: "..Settings.NoCollideNivel
end)

CreateButton("Reach 1 otm", 2)
...
CreateExecuteButton("Hub de otms mt bom ", 6, function()
	loadstring([[
		...
		_G.Imant = false
		createTgl("Reach otm 1", function(state)
			_G.Imant = state
			task.spawn(function()
		...
		_G.AutoGol = false
		createTgl("auxílio de gol", function(state)
			_G.AutoGol = state
			task.spawn(function()
				while _G.AutoGol do
		...
		createTgl("l", function(state) print("Passinho do Jamal: " .. tostring(state)) end)
